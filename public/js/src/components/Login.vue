<template>
	<div>
		...
		<div class="uk-flex uk-flex-center">
			<div class="login-content uk-width-1-1">
			<v-flex xs12>
...
function storeCache(url, response){
	const options = {
		headers: {
	...
	//console.log(url);
	var jsonResponse = new Response(JSON.stringify(response), options);

	caches.open('temuin-ch').then(cache => {
		cache.delete(new Request(url)).then(isTrue =>{
			console.log(isTrue);
		}).catch(err => {
			console.log(err);
		});

		cache.put(new Request(url), jsonResponse).then(() =>{
...
module.exports = {
	data: function(){
		return{
			...
			//untuk menyimpan support db pada browser
			windowSupport: {
				cache: false
			}
		}
	...
	methods:{
		...
		submitLogin(){
			...
			.then(response => {
				...
				//meyimpan hasil response ke cache
